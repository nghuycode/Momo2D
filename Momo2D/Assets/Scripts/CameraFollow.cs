...
public class CameraFollow : MonoBehaviour
{
    ...
    public float speedFollow, borderLeft, borderRight, borderTop, borderBot;
    void LateUpdate()
    {
        ...
        if (newPosition.y > borderTop)
            newPosition = new Vector3(newPosition.x, borderTop, newPosition.z);
        if (newPosition.y < borderBot)
            newPosition = new Vector3(newPosition.x, borderBot, newPosition.z);
